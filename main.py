...
origins = ["https://foldercopier.com", "https://www.foldercopier.com", "http://localhost:3000"]
...
async def start_copy(request: CopyRequest, db: Session = Depends(get_db)):
    ...
    payload = {"code": request.auth_code, "client_id": GOOGLE_CLIENT_ID, "client_secret": GOOGLE_CLIENT_SECRET, "redirect_uri": "postmessage", "grant_type": "authorization_code"}
    ...
    if not refresh_token:
        raise HTTPException(status_code=400, detail="Failed to acquire Refresh Token.")

    task = copy_drive_task.delay(request.source_link, request.dest_link)
    db.add(TaskToken(task_id=task.id, refresh_token=refresh_token))
    db.commit()
...
async def get_status(task_id: str):
    ...
    return {"task_id": task_id, "status": task_result.status, "info": task_result.info or {}}
...
async def get_system_stats():
    ...
    except Exception: return {"total_copies": 0}

# --- API MỚI DÀNH RIÊNG CHO ADMIN ---
@app.get("/api/admin/system-logs")
async def get_admin_logs(secret: str):
    # Thay đổi mật khẩu này nếu muốn
    if secret != "admin123":
        raise HTTPException(status_code=403, detail="Sai mật khẩu Admin")
    
    try:
        success = redis_client.get("total_successful_copies")
        failed = redis_client.get("total_failed_copies")
        errors = redis_client.hgetall("copy_errors_log")
